...
class PriceControllerTests {
	...
	void fetchAllUsersEndpoint() throws Exception {
		final MvcResult result = this.mockMvc.perform(get("/api/v1/all").contentType(MediaType.APPLICATION_JSON))
	...
	void getApplicableRanges() throws Exception {
		final CreatePriceRequest priceRequest = new CreatePriceRequest(LocalDateTime.parse("2020-06-14T10:00:00"),
				35455, "1");
		final ObjectMapper mapper = new ObjectMapper();
		mapper.registerModule(new JavaTimeModule());
		this.mockMvc
				.perform(post("/api/v1/ranges").contentType(MediaType.APPLICATION_JSON)
						.content(mapper.writeValueAsString(priceRequest)).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$[0].chainIdentifier", is(1))).andExpect(jsonPath("$[0].rateToApply", is(1)))
	...
	void testScenarios(@AggregateWith(PriceRequestAggregator.class) CreatePriceRequest createPriceRequest,
		...
		this.mockMvc
				.perform(post("/api/v1/ranges").contentType(MediaType.APPLICATION_JSON)
						.content(mapper.writeValueAsString(createPriceRequest)).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
	...
	void shouldReturnBadRequestWhenBrandIdIsInvalid() throws Exception {
		final CreatePriceRequest invalidRequest = new CreatePriceRequest(LocalDateTime.parse("2020-06-14T10:00:00"),
				35455, "invalidBrandId");
		final ObjectMapper mapper = new ObjectMapper();
		mapper.registerModule(new JavaTimeModule());

		this.mockMvc
				.perform(post("/api/v1/ranges").contentType(MediaType.APPLICATION_JSON)
						.content(mapper.writeValueAsString(invalidRequest)).accept(MediaType.APPLICATION_JSON))
